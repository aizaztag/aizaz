...
<div class="container">
    {{--new changes adding flash messages in failure and success--}}
            ...
            <div class="modal-body">
                ...
                <form id="productForm" name="productForm" class="form-horizontal" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            <input type="text" class="form-control" id="code" name="code" placeholder="Enter Tilte" value="" maxlength="50" >
                        </div>
                    </div>
...
<script>
    ...
    if ($("#productForm").length > 0) {
        $("#productForm").validate({
            submitHandler: function(form) {
                ...
                $.ajax({
                    data:   formData ,//$('#productForm').serialize(),//formData,
                    ...
                    success: function (data) {
                        ...
                        var oTable = $('#laravel_datatable').dataTable();
                        oTable.fnDraw(false);
                        showFlushSessionMessage(data.message);
                    },
                    error: function (data) {
                        console.log('Error:', data);
                        if (data.status == 422) { // when status code is 422, it's a validation
                            var obj = JSON.parse(data.responseText);
                            $('.success_message').fadeIn().html(obj.message);
                            $.each(obj.errors, function(key, value) {
                                var el = $(document).find('[name="'+key+'"]');
                                el.after($('<span style="color: red;" class="errorSpan">'+value+'</span>'));
                            });
                        }
                        $('#btn-save').html('Save Changes');
                    }
    ...
    function showFlushSessionMessage(message){
        $('.alert').hide();
        if(message.success.length !== 0){
            $('#flashSuccess').show();
            $('.flashSuccess').html(message.success);
        }else if(message.error.length !== 0){
            $('#flashError').show();
            $('.flashError').html(message.error);
        }else if(message.warning.length !== 0){
            $('#flashWarning').show();
            $('.flashWarning').html(message.warning);
        }else{
            $('#flashInfo').show();
            $('.flashInfo').html(message.info);
        }
    }

    $('#ajax-product-modal').on('hidden.bs.modal', function () {
       $('.errorSpan').remove()
    });
</script>
</html>
